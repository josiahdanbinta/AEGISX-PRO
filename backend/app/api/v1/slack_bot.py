...
from app.services.slack_bot import aegisx_slack_bot
...
class SlackTestRequest(BaseModel):
    ...
    message: str = "Test notification from AEGISX"
...
async def bot_status(current_user: dict = Depends(get_current_user)):
    return {
        "running": aegisx_slack_bot._running,
        "configured": bool(aegisx_slack_bot._bot_token),
        "socket_mode": True,
    }
...
async def test_notification(body: SlackTestRequest, current_user: dict = Depends(get_current_user)):
    alert = {
        ...
        "title": body.message,
        "severity": "info",
        "description": "This is a test notification from AEGISX SOC Platform.",
        "source_ip": "192.168.1.1",
        "hostname": "test-endpoint",
    }
    await aegisx_slack_bot.push_alert(body.channel, alert)
...
async def push_alert(alert: Dict[str, Any], channel: str = "#soc-alerts",
    ...
    await aegisx_slack_bot.push_alert(channel, alert)
